...
for pt in "${waiting_infer_pts[@]}"; do
    # if this checkpoint has been measured, skip it.
    if [ -f $WORK_PATH/tables/${EXPERIMENT_NAME}/${EXPERIMENT_ID}/${pt}.xlsx ]; then
        continue
    fi
    ...
    bash scripts/inference.sh ${EXPERIMENT_NAME} ${EXPERIMENT_ID} ${pt} ${num_gpus} ${ROOT_PATH}
    bash scripts/measure.sh ${EXPERIMENT_NAME} ${EXPERIMENT_ID} ${pt} ${ROOT_PATH}
    bash scripts/call_comet.sh ${EXPERIMENT_NAME} ${EXPERIMENT_ID} ${pt} ${ROOT_PATH} ${num_gpus}
    python scripts/make_table.py ${WORK_PATH} ${EXPERIMENT_NAME} ${EXPERIMENT_ID} ${pt}
